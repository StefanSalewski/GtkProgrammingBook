...
alternative to the gintro based ones. Besides gobject-introspection based ones other C
...
pass the button instance to the callback as first parameter. But we intend to call
...
we connect a handler (callback) proc to the named signal.
For example, a button provides the "clicked" signal, or a text entry widget
...
as second parameter an instance of type ParamSpec. This data type is not used that often --
...
about the GtkEntry widget in a later section, and we will use the notify signal of a more
